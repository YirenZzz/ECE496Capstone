...
class doubleCNN(keras.Model):
    ...
    def __init__(self):
        ...
        # Convolution
        # network = conv1d_layer(input_var=input_var, filter_size=50, n_filters=64, stride=6, wd=1e-3)
        self.conv1 = kl.Conv1D(filters=16, kernel_size=25, strides=3,
                        padding='same', use_bias=False,
        ...
        # Max pooling
        # network = max_pool_1d(input_var=network, pool_size=8, stride=8)
        self.pool1 = kl.MaxPooling2D(pool_size=(1,4), strides=(1,4),padding='same',input_shape=(1280, 1,16))
        
        # Dropout
        self.drop1 = kl.Dropout(rate=0.3)
        
        
        # Convolution
        self.conv2 = kl.Conv1D(filters=32, kernel_size=4, strides=1,
                        padding='same', activation='relu')
        self.bn2 = kl.BatchNormalization()
        self.relu2 = kl.ReLU()
        
        self.conv3 = kl.Conv1D(filters=32, kernel_size=4, strides=1,
                        padding='same', activation='relu')
        self.bn3 = kl.BatchNormalization()
        self.relu3 = kl.ReLU()
        
        self.conv4 = kl.Conv1D(filters=32, kernel_size=4, strides=1,
                        padding='same', activation='relu')
        self.bn4 = kl.BatchNormalization()
        self.relu4 = kl.ReLU()
        # Max pooling
        # self.pool2 = kl.MaxPooling1D(pool_size=4, strides=4)
        self.pool2 = kl.MaxPooling2D(pool_size=(1,2), strides=(1,2),padding='same',input_shape=(160, 1, 32))
        ...
        self.conv1_2 = kl.Conv1D(filters=8, kernel_size=200, strides=50,
                        padding='same', use_bias=False,
                        kernel_regularizer=tf.keras.regularizers.l2(1e-4))
        ...
        # Max pooling
        # network = max_pool_1d(input_var=network, pool_size=8, stride=8)
        self.pool1_2 = kl.MaxPooling2D(pool_size=(1,4), strides=(1,4),padding='same',input_shape=(1280, 1, 8))
        
        # Dropout
        self.drop1_2 = kl.Dropout(rate=0.3)
        
        # Convolution
        self.conv2_2 = kl.Conv1D(filters=16, kernel_size=4, strides=1,
                        padding='same', activation='relu')
        self.bn2_2 = kl.BatchNormalization()
        self.relu2_2 = kl.ReLU()
        
        self.conv3_2 = kl.Conv1D(filters=16, kernel_size=4, strides=1,
                        padding='same', activation='relu')
        self.bn3_2 = kl.BatchNormalization()
        self.relu3_2 = kl.ReLU()
        
        self.conv4_2 = kl.Conv1D(filters=16, kernel_size=4, strides=1,
                        padding='same', activation='relu')
        self.bn4_2 = kl.BatchNormalization()
        self.relu4_2 = kl.ReLU()
        # Max pooling
        # self.pool2 = kl.MaxPooling1D(pool_size=4, strides=4)
        self.pool2_2 = kl.MaxPooling2D(pool_size=(1,2), strides=(1,2),padding='same',input_shape=(160, 1, 16))
...
class doubleCNNBiLSTM(keras.Model):
    ...
    def __init__(self, output_dim, lstm_units):
        ...
        # Max pooling
        # network = max_pool_1d(input_var=network, pool_size=8, stride=8)
        self.pool1 = kl.MaxPooling2D(pool_size=(1,4), strides=(1,4),padding='same',input_shape=(1280, 1, 8))
        ...
        # Convolution
        self.conv2 = kl.Conv1D(filters=16, kernel_size=6, strides=1,
                        padding='same', activation='relu')
        self.bn2 = kl.BatchNormalization()
        self.relu2 = kl.ReLU()
        
        self.conv3 = kl.Conv1D(filters=16, kernel_size=6, strides=1,
                        padding='same', activation='relu')
        self.bn3 = kl.BatchNormalization()
        self.relu3 = kl.ReLU()
        
        self.conv4 = kl.Conv1D(filters=16, kernel_size=6, strides=1,
                        padding='same', activation='relu')
        self.bn4 = kl.BatchNormalization()
        self.relu4 = kl.ReLU()
        # Max pooling
        # self.pool2 = kl.MaxPooling1D(pool_size=4, strides=4)
        self.pool2 = kl.MaxPooling2D(pool_size=(1,2), strides=(1,2),padding='same',input_shape=(160, 1, 16))
    ...
    def call(self, input, training=True):
        ...
        print('x cocat',x.shape) 
        x = self.drop1(x)
        print('x cocat drop1', x.shape)
        x = Reshape(target_shape=(64, 85))(x)
        
        print('x reshape',x.shape) 
        
        # BiLSTM layers
        x = self.bi_lstm1(x)
        print('x shape lstm1',x.shape) 
        x = self.drop1(x)
        x = self.bi_lstm2(x)
