...
def run(batch_size, pretrain_epochs):
    ...
    for i, (trainfile_idx, testfile_idx) in enumerate(kfold.split(allfiles)):
        ...
        # if parameters.load_pretain_model == False:
        if parameters.model_mode == 'simpleCNN':
            model = simpleCNN()
            
        if parameters.model_mode == 'doubleCNN':
            model = doubleCNN()
            model = keras.models.load_model(parameters.saved_pretain_model)
        ...
        # preprocessed_train_data = model.predict(x_train)
        # preprocessed_val_data = model.predict(x_val)
        
        # final_model = BiLSTM(32,input_shape=(5, 2560))
        # define BiLSTM model
        
        
        final_model = doubleCNNBiLSTM(output_dim=5,lstm_units=32)
        final_model.build((None, 1,7680,1))
        # model.summary()
        final_model.summary()
        # Copy the pre-trained weights to the corresponding layers in the fine-tuning model
        for i, layer in enumerate(model.layers[:-1]):
            # print(i)
            if model.layers[i].get_weights():
                final_model.layers[i].set_weights(model.layers[i].get_weights())
            # print(final_model.layers[i].get_weights())
        
        final_model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
        
        final_model.fit(x=x_train, y=y_train, epochs=pretrain_epochs, batch_size=batch_size)
        
        val_loss_f, val_acc_f = final_model.evaluate(x_val, y_val)

        all_scores_f.append(val_acc_f)
        
        if val_acc_f > best_acc:
            best_acc = val_acc_f
            best_model = model
    ...
    print("Validation accuracy final:", np.mean(all_scores_f))
    # save the best model
    ...
    best_model.save("result/output/bestModel/best_Final_model")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_mode', type=str, default='doubleCNN',choices=['simpleCNN', 'doubleCNN', 'fashionCNN'])
    parser.add_argument('--load_pretain_model', type=bool, default=True)
    parser.add_argument('--final_tune', type=bool, default=True)
    parser.add_argument('--saved_pretain_model', type=str, default='result/output/bestModel/best_doubleCNNmode20230402_1')
